...
public class Reporter : MonoBehaviour
{
	...
	public class Sample
	{
		...
		{
			if ((int)loadedScene == -1 || (int)loadedScene >= scenes.Length)
	...
	public void Initialize()
	{
		if (!created) {
            ...
            Debugger.LogWarning("tow manager is exists delete the second");
		...
		size.x = size.y = PlayerPrefs.GetFloat("Reporter_size", 18);
	...
	void DrawLogs()
	{
		...
		for (int i = startIndex; (startIndex + index) < endIndex; i++) {
			...
			if (log == selectedLog) {
                ...
                log.condition = log.condition.Length > 700 ? log.condition.Substring(0, 700): log.condition;
                if (GUILayout.Button(log.condition, logButtonStyle)) {
                    //selectedIndex = startIndex + index ;
                    GameLogger.Log(condition);
                    selectedLog = log;
				}
				//GUILayout.FlexibleSpace();
				if (showTime) {
	...
	void AddLog(string condition, string stacktrace, LogType type)
	{
		...
		if (TotalMemUsage > maxSize) {
			clear();
            Debugger.Log("Memory Usage Reach" + maxSize + " mb So It is Cleared");
			return;
		}
...
#if UNITY_CHANGE3
		currentScene = SceneManager.GetActiveScene().name ;
        Debugger.Log( "Scene " + SceneManager.GetActiveScene().name + " is loaded");
#else
		currentScene = Application.loadedLevelName;
		Debugger.Log("Scene " + Application.loadedLevelName + " is loaded");
#endif
    }
